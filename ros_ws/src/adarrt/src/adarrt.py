...
class AdaRRT(object):
    ...
    def build(self):
        ...
        for k in range(self.max_iter):
            ...
            if new_node and self._check_for_completion(new_node):
                self.goal = AdaRRT.Node(self.goal.state, new_node)
    ...
    def _get_random_sample(self):
        ...
        p=np.zeros(len(self.goal.state))
        for i in range(len(self.goal.state)):
            temp=np.linspace(-8,8,2000)
    ...
    def _get_nearest_neighbor(self, sample):
        ...
        for iter in self.start:
            ...
            if(d<d_min):   #find the nearest neighbor
                d_min=d
                node_neighbor=iter
...
def main():
    ...
    bowlURDFUri="package://pr_assets/data/objects/bowl.urdf"
    bowlPose=[0.3, 0.0, 0.02, 0.707107, 0, 0, 0.707107]
    bowlPose2=[0.2, -0.6, 0.02, 0.707107, 0, 0, 0.707107]
    world = ada.get_world()
    can = world.add_body_from_urdf(canURDFUri, sodaCanPose)
    table = world.add_body_from_urdf(tableURDFUri, tablePose)
    bowl1=world.add_body_from_urdf(bowlURDFUri,bowlPose)
    bowl2=world.add_body_from_urdf(bowlURDFUri,bowlPose2)

    # add collision constraints
    collision_free_constraint = ada.set_up_collision_detection(
            ada.get_arm_state_space(),
            ada.get_arm_skeleton(),
            [can, table,bowl1,bowl2])
